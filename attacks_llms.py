...
class GeneticAtackLLMS(object):
    ...
    def attack(self, x_orig, target, max_change=0.4):
        ...
        return None
        @staticmethod
    def get_predictions(self, texts, target, labels=['Positive', 'Negative'], model_name='text-davinci-003'):
        prompts = ['What is the sentiment of the following review?']
        ...
        for text in texts:
            ...
            for user_message in prompts:
                ...
                """)
                output = prompt(text=text, options=labels, user_message=user_message)
                all_resp['answer'].append(output['answer'])
                all_resp['logprobs'].append(output['logprobs'])
            probs.append(mean([k[mapping[target]] for k in all_resp['logprobs']]))
